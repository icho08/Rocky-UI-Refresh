    ...
    private void renderHudInternal(HudEvent event) {
        ...
        try {
            if (armorHud.getValue() && mc.player != null) {
                ...
                ctx.fill(bx + 3, by, bx + bw, by + headH,
                        GuiTheme.rgba(ac.getRed(), ac.getGreen(), ac.getBlue(), 18));
                ctx.fill(bx + 3, by + headH - 1, bx + bw, by + headH,
                        GuiTheme.rgba(ac.getRed(), ac.getGreen(), ac.getBlue(), 60));
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    // fakeItem renders the item icon; guard individually in case the
                    // item-renderer isn't ready during extractRenderState.
                    try { ctx.fakeItem(stack, bx + 5, rowY + 1); } catch (Exception ignored2) {}
                    ...
                    if (iconId != null) {
                        try {
                            Identifier spriteId = Identifier.fromNamespaceAndPath(
                                    iconId.getNamespace(), "mob_effect/" + iconId.getPath());
